___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Matomo Analytics - Actions",
  "categories": ["ANALYTICS", "TAG_MANAGEMENT"],
  "brand": {
    "id": "brand_dummy",
    "displayName": "data-marketing-school",
    "thumbnail": "data:image/png;base64,"
  },
  "description": "",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "RADIO",
    "name": "selectedAction",
    "displayName": "Actions",
    "radioItems": [
      {
        "value": "page_view",
        "displayValue": "Page View"
      },
      {
        "value": "event",
        "displayValue": "Event",
        "subParams": [
          {
            "type": "TEXT",
            "name": "eventCategory",
            "displayName": "Category (Required)",
            "simpleValueType": true,
            "valueHint": "Reviews",
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          },
          {
            "type": "TEXT",
            "name": "eventAction",
            "displayName": "Action (Required)",
            "simpleValueType": true,
            "valueHint": "Published Matomo Review",
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          },
          {
            "type": "TEXT",
            "name": "eventName",
            "displayName": "Name (Optional)",
            "simpleValueType": true,
            "valueHint": "new_review",
            "valueValidators": []
          },
          {
            "type": "TEXT",
            "name": "eventValue",
            "simpleValueType": true,
            "valueHint": "5",
            "valueValidators": [],
            "displayName": "Value (Optional)"
          }
        ]
      },
      {
        "value": "goal",
        "displayValue": "Goal",
        "subParams": [
          {
            "type": "TEXT",
            "name": "goalId",
            "displayName": "Goal ID (Required)",
            "simpleValueType": true,
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              },
              {
                "type": "POSITIVE_NUMBER"
              }
            ],
            "valueHint": "1"
          },
          {
            "type": "TEXT",
            "name": "customRevenue",
            "displayName": "Custom Revenue (Optional)",
            "simpleValueType": true,
            "valueHint": "17.99",
            "valueValidators": []
          }
        ]
      },
      {
        "value": "link",
        "displayValue": "Link",
        "subParams": [
          {
            "type": "TEXT",
            "name": "linkUrl",
            "displayName": "Link URL (Required)",
            "simpleValueType": true,
            "valueHint": "https://example.com/xyz",
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          },
          {
            "type": "SELECT",
            "name": "linkType",
            "displayName": "Link Type (Required)",
            "macrosInSelect": false,
            "selectItems": [
              {
                "value": "link",
                "displayValue": "Outlink"
              },
              {
                "value": "download",
                "displayValue": "Download"
              }
            ],
            "simpleValueType": true,
            "valueValidators": [
              {
                "type": "NON_EMPTY"
              }
            ]
          }
        ]
      },
      {
        "value": "site_search",
        "displayValue": "Site Search",
        "subParams": [
          {
            "type": "TEXT",
            "name": "searchKeyword",
            "displayName": "Keyword (Optional - Recommended)",
            "simpleValueType": true,
            "valueHint": "matomo site search"
          },
          {
            "type": "TEXT",
            "name": "searchCategory",
            "displayName": "Category (Optional - Recommended)",
            "simpleValueType": true,
            "valueHint": "Blog posts"
          },
          {
            "type": "TEXT",
            "name": "searchResultsCount",
            "displayName": "Results count (Optional - Recommended)",
            "simpleValueType": true,
            "valueValidators": [
              {
                "type": "NON_NEGATIVE_NUMBER"
              }
            ],
            "valueHint": "14"
          }
        ]
      }
    ],
    "simpleValueType": true
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// Enter your template code here.
const log = require('logToConsole');
const copyFromWindow = require("copyFromWindow");
const callInWindow = require("callInWindow");

function isMatomoSet() {
  const _paq = copyFromWindow("_paq");
  if (_paq) {
    return true;
  }
  log("Error: _paq variable not set, \"Matomo Analytics - Actions\" should only be triggered after \"Matomo Analytics - Configuration\" tag.");
  data.gtmOnFailure();
  return false;
}

if(isMatomoSet()) {
   
  if(data.selectedAction == "page_view") {
    callInWindow("_paq.push", ["trackPageView"]);
  }

  if(data.selectedAction == "event") {
    callInWindow("_paq.push", ['trackEvent', data.eventCategory, data.eventAction, data.eventName, data.eventValue]);
  }

  if(data.selectedAction == "goal") {
    callInWindow("_paq.push", ['trackGoal', data.goalId, data.customRevenue]);
  }

  if(data.selectedAction == "link") {
    callInWindow("_paq.push", ["trackLink", data.linkUrl, data.linkType]);
  }

  if(data.selectedAction == "site_search") {
    callInWindow("_paq.push", ["trackSiteSearch", data.searchKeyword, data.searchCategory, data.searchResultsCount]);
  }

}

// Call data.gtmOnSuccess when the tag is finished.
data.gtmOnSuccess();


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "_paq.push"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "_paq"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 01/01/2024, 10:04:31


